...
const generateIdZone = (pattern, row, col, layer) => {
	...
	const regex = new RegExp(/[(][0-9a-z+-/*$\s]+[)]/g);
	...
	if (matches)
		matches.forEach((match) => {
			...
			let matchReplaced = match.replace("$rr$", row);
			matchReplaced = matchReplaced.replace("$r$", row);
			matchReplaced = matchReplaced.replace("$cc$", col);
			matchReplaced = matchReplaced.replace("$c$", col);
			matchReplaced = matchReplaced.replace("$ll$", layer);
			matchReplaced = matchReplaced.replace("$l$", layer);

			const result = zeroPad(
				mathjs.evaluate(matchReplaced),
				zeroPadTwo ? 2 : 1
			);
	...
	generateZones(context) {
		...
		for (let countRows = 0; countRows < rows; countRows++) {
			for (let countCols = 0; countCols < columns; countCols++) {
				...
				newZone.PosYMin = initY + countRows * (length + rowGap);
				newZone.PosYMax =
					initY + countRows * (length + rowGap) + length;
