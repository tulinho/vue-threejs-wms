	...
	axios
		...
		.then(function(response) {
			...
			context.rootState.yard.sections.forEach((section) => {
				...
				section = Object.assign({}, defaults.section, section);
				section.Section = section.Zone;
				let sql = formatSectionScript(
					section,
					sqlScripts.sectionScript
				);
				script.push(sql);
				sql = formatZoneScript(section, sqlScripts.zoneScript);
				script.push(sql);
...
function exportZones(context) {
	...
	axios
		...
		.then(function(response) {
			let allZones = response.data.value;
			let existingZones = allZones.filter(
				(m) =>
					m.ZoneType == "Z" ||
					m.ZoneType == "V" ||
					m.ZoneType == "D" ||
			...
			zonesToUpdate.forEach((zone) =>
				promises.push(updateZone(context, zone))
			);
			zonesToInsert.forEach((zone) =>
				promises.push(insertZone(context, zone))
			);
			Promise.all(promises)
				.then((m) => {
					let message = m.map((m) => m.status).every((m) => m == 204)
						? "Success"
						: "Error";
...
function formatZoneProperties(zone) {
	...
	zone.GenEventEnter = !!zone.GenEventEnter;
	zone.GenEventExit = !!zone.GenEventExit;
}
